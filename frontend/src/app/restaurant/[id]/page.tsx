...
export default function RestaurantDetailPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (!isNaN(numericId)) {
        const res = await apiFetchJson<Restaurant>(`/api/v1/restaurants/${numericId}`);
        if (res.ok && res.data) {
  ...
  if (error || !restaurant) {
    return (
      <AppShell>
        <p className="py-20 text-center text-sm text-red-500">{error || "식당을 찾을 수 없습니다."}</p>
      </AppShell>
    );
  }
    ...
    >
      <div className="space-y-5">
        ...
        <nav className="flex items-center gap-2 text-sm text-muted">
          ...
          <span>/</span>
          <Link href={`/search?location=${restaurant.region1}`} className="hover:text-primary">{restaurant.region1}</Link>
          <span>/</span>
          <Link href={`/search?category=${restaurant.category}`} className="hover:text-primary">{restaurant.category}</Link>
          <span>/</span>
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm text-muted">
                  포스트 <span className="font-bold text-ink">{feeds.length}</span>개
                </p>
              </div>
            ...
            <div className="mt-5 space-y-2.5 text-sm text-body">
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  ...
                  {restaurant.roadAddress && <p className="text-muted-soft">{restaurant.address}</p>}
                </div>
              </div>
        ...
        <div>
          ...
          {feeds.length === 0 ? (
            <p className="py-10 text-center text-sm text-muted">아직 작성된 포스트가 없습니다.</p>
          ) : (
            <div className="space-y-4">
              {feeds.map((post) => (
                <article key={post.feedId} className="rounded-2xl bg-surface p-5 border border-hairline-soft">
